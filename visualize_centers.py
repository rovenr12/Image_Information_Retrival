...
for (i, imageID) in enumerate(featuresDB["image_ids"]):
    ...
    for (kp, features) in zip(kps, descs):
        # Compute the distance between the feature vector and all clusters (one for each)
        features = features.reshape(1, -1)
        D = pairwise.euclidean_distances(features, Y=vocab)[0]

        # Loop over the distances dictionary
        for j in np.arange(0, len(vocab)):
            # Grab the set of top visualization results for the current visual word and
            # update the top results with a tuple of the distance, keypoint, and image ID
