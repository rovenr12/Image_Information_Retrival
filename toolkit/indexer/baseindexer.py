...
class BaseIndexer(object):
    ...
    def _writeBuffer(self, dataset, datasetName, buf, idxName, sparse=False):
        # If the buffer is a list, then compute the ending index based on the list's length
        if type(buf) is list:
            end = self.idxs[idxName] + len(buf)

        # Else, assume that the buffer is a NumPy/SciPy array, so compute the ending index based on the array shape
        else:
        ...
        # If this is a sparse matrix, then convert the sparse matrix to a dense one so it can be written to file
        if sparse:
            buf = buf.toarray()
    ...
    def _resizeDataset(self, dataset, dbName, baseSize=0, finished=0):
        ...
        # Check to see if we are finished writing rows to the dataset, and if so, make the new size the current index
        if finished > 0:
            newSize = finished
