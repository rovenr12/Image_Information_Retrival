...
class SpatialVerifier:
    ...
    def rerank(self, queryKps, queryDescs, searchResult, numResults=10):
        ...
        for (i, (start, end)) in zip(resultIdxs, self.featuresDB["index"][resultIdxs, ...]):
            ...
            # Assuming that at least some keypoints were matched, the final score
        ...
        # If no spatially verified matches were found, return the initial search result object
        if len(reranked) == 0:
            return searchResult
        ...
        for (score, imageID, imageIdx) in searchResult.results:
            # Only add the initial result to the list of results if the image has not been spatially verified
    ...
    def match(self, kpsA, featuresA, kpsB, featuresB):
        # Compute the raw matches and initialize the list of actual matches and the list of inlier indexes
        ...
        if len(matches) >= self.minMatches:
            ...
            # Grab the inlier points (status > 0), then quantize the features associated with these points
